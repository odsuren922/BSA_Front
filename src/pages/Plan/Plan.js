...
const TableComponent = () => {
        ...
        const hasEmptyFields = data.some((task, taskIndex) => {
            ...
            return task.subtasks.some((subtask, subIndex) => {
                if (
                    ...
                    toast.error(
                        `Мөр #${taskIndex + 1}-ын дэд ажил #${
                            subIndex + 1
                        }-т нэр эсвэл огноо хоосон байна.`
                    );
                    return true;
                }
                return false;
    ...
    const handleDeleteSubProject = (rowIndex) => {
        ...
        if (subtasks.length > 1) {
            ...
            if (
                lastSubtask?.id &&
                !String(lastSubtask.id).startsWith("temp-")
            ) {
    ...
    const handleUnsubmit = async () => {
        ...
        try {
            ...
            if (error.response && error.response.status === 403) {
                toast.error(
                    "Багш төлөвлөгөөг зөвшөөрсөн тул буцаах боломжгүй."
                );
            } else {
    ...
    const handlepDF = async () => {
        ...
        try {
            ...
            if (
                ...
                toast.error(
                    "Төлөвлөгөөний мөчлөг олдсонгүй тул PDF үүсгэж чадахгүй."
                );
                return;
            }
