...
const TableComponent = () => {
  ...
  const { user } = useAuth();
  const { id } = useParams();
  const thesisId = user.role === "student" ? user.thesis.id : id;
  ...
  const fetchtasks = async () => {
    setLoading(true);
    try {
      const [Thesis] = await Promise.all([api.get(`/onethesis/${thesisId}`)]);
      console.log("Thesis", Thesis.data);

      setData(Thesis.data.data.tasks);
      setThesis(Thesis.data.data);
      setPlanStatus(Thesis.data.data.thesisPlanStatus);
      console.log(Thesis.data.data.thesisPlanStatus);
    } catch (error) {
  ...
  const handlepDF = async () => {
    ...
    try {
      ...
      generatePDF(data, thesis_info, thesis.thesis_cycle);
    } catch (error) {
    ...
    <div>
      {loading ? (
        ...
        <Row>
         ...
         7 хоногийн үйлчлэлсэн төлөвлөгөө</h4>
         </div>

          <Col xl={8} style={{ marginBottom: "10px" }}>
            <Card>
              <CardBody>
                {thesis && thesis.thesis_cycle ? (
                  <div
                    style={{
                      display: "flex",
                      flexDirection: "row",
                  ...
                  >
                    <div>
                      ...
                      <Tag color="blue">{thesis.thesis_cycle.start_date}</Tag>
                    </div>
                    <div>
                      <strong>Дуусах өдөр:</strong>{" "}
                      <Tag color="red">{thesis.thesis_cycle.end_date}</Tag>
                    </div>
