...
const AboutThesisTabs = ({ id, gradingSchema, thesisCycle, score, tasks,supervisor }) => {
  ...
  const approveFile = async (fileId) => {
    try {
      ...
      const a = await api.put(`/thesis/files/${fileId}/approve`, {
        accepted_by: user.id, // send user id manually
      });
      console.log(a);
          ...
          children: (
            <div>
              <Col xs={24} md={24}>
                ...
                <ThesisScores thesisId={id} thesisCycle={thesisCycle} supervisor={supervisor} />
              </Col>
            </div>
          ),
        },
          ...
          children: (
            ...
            {tasks.length > 0 ? (
                <Col xl={24} style={{ marginBottom: "10px" }}>
                  <Card>
                    <GanttChart tasks={convertToGanttTasks(tasks)} />;
                  </Card>
                </Col>
            ) 
            :(
              ...
              <a className="btn btn-primary btn-sm" href={`/studentPlan/${id}`}>
                Дэлгэрэнгүй харах
              </a>
            </div>
          ),
